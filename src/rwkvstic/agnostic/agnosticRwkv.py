...
def AgnosticRWKV(ops: module, *args):
    class myRWKV(ops.module):
        ...
        def processLayerx(self, k, v, rz: List[torch.Tensor], state, xx: int, i: int):
            ...
            a = self.add(self.multiply(e1, (state[2])), self.multiply(e2, v[i]))

            b = self.add(self.multiply(e1, (state[3])), e2)

            wwn = self.add((state[4]), self.time_decay[xx])
            ...
            outb = self.add(self.multiply(e11, (state[2])), self.multiply(e21, v[i]))

            outc = self.add(self.multiply(e11, (state[3])), e21)

            # state[2:5] = ops.stack((outb, outc, p1))

            state = self.scatter(
                state, self.scatterindices[0], self.stack((outb, outc, p1))
            )
            wkv = self.divide(a, b)
        ...
        def processLayer(self, k, v, rz: List[torch.Tensor], state, xx: int, i: int):
            ...
            wrd = self.divide(
                self.add(
                    state[2],
                    self.multiply(
                        self.multiply(ki, v[i]), self.exp(self.time_first[xx])
                    ),
                ),
                self.add(state[3], self.multiply(ki, self.exp(self.time_first[xx]))),
            )

            state = self.scatter(
                state,
                self.scatterindices[1],
                self.multiply(
                    self.exp(self.time_decay[xx]),
                    self.add(state[2:4], self.stack((self.multiply(v[i], ki), ki))),
                ),
            )
        ...
        def doLayer(self, x, state, xx: int):
            ...
            k = self.matvec(self.key[xx], self.lerp(tc, xy, self.kktk[xx]))

            v = self.matvec(self.value[xx], self.lerp(tc, xy, self.vvtv[xx]))

            rr = self.matvec(self.receptance[xx], self.lerp(tc, xy, self.rrtr[xx]))
            ...
            mvv = self.add(
                x, self.matvec(self.outputvv[xx], self.multiply(r, self.stack(rz)))
            )
            ...
            state = self.scatter(
                state, self.scatterindices[2], self.stack((xy[-1], ddd[-1]))
            )

            km = self.relu(
                self.matvec(
                    self.key_ffn[xx], self.lerp(rc, ddd, self.time_mix_k_ffn[xx])
                )
            )
            ...
            rt = self.logistical(
                (
                    self.matvec(
                        ...
                        self.lerp(rc, ddd, self.time_mix_r_ffn[xx]),
                    )
                )
            )
            ...
            x = self.add(mvv, self.multiply(rvm, rt))
        ...
        def forward(self, x, state):
            ...
            x = self.layernorm(self.processEmbed(g), self.emb1, self.emb2)

            for i in self.rng(self.len(state)):

                x, rstate = self.doLayer(x, state[i], i)
                state = self.scatter(state, i, rstate)

            x = self.matvec(
                self.postprocess2,
                self.layernorm(x, self.postprocess0, self.postprocess1),
            )
