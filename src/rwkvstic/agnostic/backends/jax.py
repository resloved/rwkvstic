...
class RWKVNumpyOps(RWKVOp.module):
    def __init__(self, layers, embed, *args, **kwargs):
        ...
        def ln(x, w, b):
            ...
            x2 = self.sqrt(
                self.mean(xee2 * xee2, axis=1, keepdims=True) + 0.000009999999747378752
            )
...
class RWKVCuPyOps(RWKVOp.module):
    def __init__(self, layers, embed, *args, **kwargs):
        ...
        with np.cuda.Device(0):
            ...
            self.initTensor = (
                lambda x: np.array(x.float().cpu().numpy(), dtype=dtype)
                if len(x.squeeze().shape) == 1
                else np.array(x.float().cpu().numpy(), dtype=mattype).T
            )
            ...
            def getIndex(x, y):
                return np.stack([x[z] for z in y])
            ...
            def ln(x, w, b):
                xee2 = x - np.mean(x, axis=1, keepdims=True)

                x2 = np.sqrt(
                    np.mean(np.square(xee2), axis=1, keepdims=True)
...
class RWKVCuPyQuantOps(RWKVCuPyOps):
    def __init__(self, layers, embed, *args, **kwargs):
        ...
        def initTensor(x):
            ...
            xx = [QuantizeMatrix(x) for x in splitmatrices]
...
class RWKVJaxOps(RWKVOp.module):
    def __init__(self, layers, embed, *args, dtype=None, preJax=False, **kwargs):
        ...
        def getIndex(x, y):
            return npjax.array([x[z] for z in y])
