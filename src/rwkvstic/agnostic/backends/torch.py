...
class RWKVPTOps(RWKVOp.module):
    def __init__(self, layers, embed, *args, dtype=None, **kwargs):
        ...
        # safe log
        def og(x):
            return torch.complex(x, torch.zeros_like(x)).log()
        ...
        def arrayGet(x: List[torch.Tensor], i: int):
            return x[i]
        ...
        def ln(x, w, b):
            ...
            x2 = torch.sqrt(torch.mean(xee2 * xee2, dim=1, keepdim=True) + 1e-5)
            o = w * (xee2 / x2) + b
...
class RWKVCudaOps(RWKVPTOps):
    def __init__(
        self, layers, embed, *args, useGPU=None, runtimedtype=None, dev="cuda", **kwargs
    ):
        ...
        useGPU = (
            inquirer.confirm("Use GPU?", default=True) if useGPU is None else useGPU
        )
            ...
            inquirer.prompt(
                [
                    inquirer.List(
                        ...
                        choices=[torch.bfloat16, torch.float32, torch.float64],
                    )
                ]
            )["type"]
            if runtimedtype is None
            else runtimedtype
        )
        ...
        def initTensor(x):
            ret = x.to(dtype=self.dtype if len(x.shape) > 1 else runtimedtype).to(
        ...
        def matvec(x, y):
            return y.to(dtype=x.dtype).matmul(x).to(dtype=y.dtype)
...
class RWKVPTTSExportOps(RWKVCudaOps):
    def __init__(self, layers, embed, *args, includeSampler=None, **kwargs):
        ...
        includeSampler = (
            inquirer.confirm("Include sampler?", default=True)
            if includeSampler is None
            else includeSampler
        )
...
class RWKVCudaDeepspeedOps(RWKVCudaOps):
    def __init__(self, layers, embed, *args, **kwargs):
        ...
        self.postProcessModule = lambda x: deepspeed.init_inference(
            x, replace_method="auto", replace_with_kernel_inject=True
        ).module
    ...
    def __init__(
        ...
        **kwargs,
        ...
        def QuantizeMatrix(x, runtimeDtype, device, stream):
            ...
            x = (x - mini) / ran
            if stream:
                x = x.to(dtype=torch.uint8, non_blocking=True).pin_memory()
            else:
                x = x.to(dtype=torch.uint8, non_blocking=True, device=device)

            return [
                x,
                ran.to(runtimeDtype).to(device=device),
                mini.to(runtimeDtype).to(device=device),
            ]

        def QuantizedMatVec(x, y, runtimedtype):
            if len(x) != 3:
                return (
                    x.to(device=y.device, non_blocking=True)
                    .matmul(y.to(dtype=x.dtype).t())
                    .to(dtype=y.dtype)
                    .t()
                )
            rx, spread, zpoint = x
            ...
            xmain = yy.matmul(
                rx.to(dtype=dtype, device=y.device, non_blocking=True)
            ).to(dtype=runtimedtype)
        ...
        target = (
            float(inquirer.text("Target size (in GB):", default="100"))
            if target is None and dev == "cuda"
            else target
        )

        maxQuantTarget = (
            float(inquirer.text("Max quantization target size (in GB):", default="100"))
            if maxQuantTarget is None and dev == "cuda"
            else maxQuantTarget
        )
        ...
        def initTensor(x):
            ...
            dostream = (
                (torch.cuda.max_memory_reserved(0) / 1024 / 1024 / 1024 > target)
                if dev == cuda
                else False
            )
            ...
            if preQuantized and len(x) == 3:
                return (
                    x[0].to(device=dev),
                    x[1].to(dtype=runtimedtype, device=dev),
                    x[2].to(dtype=runtimedtype, device=dev),
                )
            ...
            xx = [QuantizeMatrix(x, runtimedtype, dev, dostream) for x in splitmatrices]
        ...
        self.initTensor = initTensor
        self.stack = lambda x: torch.stack(x) if isinstance(x[0], torch.Tensor) else x
...
class RWKVStreamBigOps(RWKVCudaQuantOps):
    def __init__(self, layers, embed, *args, **kwargs):
        ...
        super().__init__(
            layers, embed, *args, useGPU=True, chunksize=1, maxQuantTarget=-1, **kwargs
        )
...
class RWKVSplitCudaOps(RWKVPTOps):
    ...
    def __init__(
        ...
        **kwargs,
        ...
        self.initTensor = (
            lambda x: x.to(dtype=runtimedtype).cuda()
            if len(x.shape) == 1
            else list(
                map(
                    lambda zx: zx[1].to(
                        device=devices[zx[0]],
                        dtype=torch.float32
                        if "cpu" in devices[zx[0]]
                        else torch.bfloat16,
                    ),
                    enumerate(list(x.chunk(len(devices), dim=1))),
                )
            )
        )
        self.initCpuTensor = lambda x: x.to(dtype=runtimedtype)
        ...
        def matvec(matx, y):
            chunks = list(
                map(
                    lambda xx: xx[1].to(
                        device=devices[xx[0]],
                        dtype=matx[xx[0]].dtype,
                        non_blocking=True,
                    ),
                    enumerate(y.chunk(len(devices), dim=0)),
                )
            )
            res = (
                matx[0]
                .mv(chunks[0])
                .to(dtype=runtimedtype, device=y.device, non_blocking=True)
            )
