    ...
    def __init__(self, layers, embed, *args, dtype=None, **kwargs):
        ...
        if dtype is None:
            ...
            questions = [
                inquirer.List(
                    ...
                    choices=[torch.float32, torch.float16],
                ),
            ]
        ...
        def initTensor(x):
            ...
            if isinstance(x, list):
                ...
                # convert to float32
                xx = xx.astype(nptype)
            rrx = onnx.helper.make_tensor(name, dtype, xx.shape, xx.tobytes(), raw=True)
        ...
        def ppm(x):
            inputtensor = (
                onnx.helper.make_tensor_value_info(
                    "input0", onnx.TensorProto.INT32, [1]
                ),
                "input0",
            )
            ...
            emptyState = list(
                map(
                    lambda x: (
                        onnx.helper.make_tensor_value_info(
                            "instate" + str(x), dtype, [embed]
                        ),
                        "instate" + str(x),
                    ),
                    range((4 + self.useLogFix) * layers),
                )
            )
            ...
            print(outs)
            logits = onnx.helper.make_tensor_value_info(outs[0], dtype, [50277])
            state = list(
                map(
                    lambda x: onnx.helper.make_tensor_value_info(x, dtype, [embed]),
                    outs[1],
                )
            )
