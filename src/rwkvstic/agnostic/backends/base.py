...
class module:
    def __init__(self, layers, embed, *args, useLogFix=True, **kwargs):
        ...
        self.MatVec = (self.MatrixType, self.VectorType), self.VectorType

        def raiseNotImplemented(
            *args: List[self.TensorType], **kwargs: Dict[str, self.TensorType]
        ) -> self.TensorType:
        ...
        self.emptyState: self.MatrixType = np.array(
            [
                (
                    (
                        [[0.00] * embed, [0.00] * embed, [0.00] * embed, [0.00] * embed]
                        + ([[-1e30] * embed] if self.useLogFix else [])
                    )
                )
            ]
            * layers
        ...
        def arrayPush(x: list, y, i: int):
            return [*x[0:i], y, *x[i + 1 :]]
