...
def loadWeights(mode, path, *args, processEmb=True, **kwargs):
    ...
    w: Dict[str, torch.Tensor] = torch.load(path, map_location="cpu")
    # refine weights
    keys = list(w.keys())
    for x in keys:
    ...
    if kwargs.get("lora_r", 0) > 0:
        ...
        if kwargs.get("lora_bake", None) is None:
            kwargs["lora_bake"] = inquirer.confirm("bake lora into weights?")
        if kwargs["lora_bake"]:
            ...
            if ops.stackEmb:
                ...
                if processEmb:
                    w[x] = ops.stack(
                        list(
                            map(
                                lambda rrx: ops.initCpuTensor(rrx.squeeze()),
                                w[x].split(1, 0),
                            )
                        )
                    )
